...
import com.meter.sdk.utils.crypto.ECIESUtils;
...
public class ECIESTest {
	...
	public void test() throws Exception {
		...
		ECPoint bobPoint = ECKey.publicPointFromPrivate(
		...
		ECPoint bobPoint1 = ECIESUtils
				.createECPointFromPublicKey(ByteUtils.toHexString(bobKey.getRawPublicKey(false)));
		...
		// remove prefix
		byte[] Rencoded2 = BytesUtils.toBytesPadded(
				new BigInteger(1, Arrays.copyOfRange(Rencoded, 1, Rencoded.length)),
				64);
		...
		byte[] Pencoded2 = BytesUtils.toBytesPadded(
				new BigInteger(1, Arrays.copyOfRange(Pencoded, 1, Pencoded.length)),
				64);
		...
		System.out.println(
				ByteUtils.toHexString(Rencoded) + ByteUtils.toHexString(cBytes)
						+ ByteUtils.toHexString(dBytes));

		// 解密
		System.out.println("解密======================");
		System.out.println("1. 计算共享密钥，S=Px。 P=(Px,Py)=kb*R=kb*r*G=r*kb*G=r*KB");
		byte[] p_RBytes = BytesUtils.toBytesPadded(
				new BigInteger(1, Arrays.copyOfRange(Rencoded, 1, Rencoded.length)),
				64);
		...
		byte[] P_client_bytes_without_prefix = BytesUtils
				.toBytesPadded(new BigInteger(1,
						Arrays.copyOfRange(P_client_bytes, 1, P_client_bytes.length)), 64);
	...
	public void testECIES() throws Exception {
		...
		System.out.println(
				"original file content hash:" + BytesUtils.toHexString(plainHash, Prefix.ZeroLowerX));
