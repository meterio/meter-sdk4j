...
import com.meter.sdk.utils.crypto.ECDSASignature;
...
public class X509CertificateUtilsTest extends BaseTest {
	...
	public void testParseCertificate() {
		...
		System.out.println(
				"SN: " + BytesUtils.toHexString(certificate.getSerialNumber().toByteArray(),
						Prefix.ZeroLowerX));
		System.out.println("Sig: " + BytesUtils.toHexString(certificate.getSignature(), Prefix.ZeroLowerX));

		System.out
				.println("PUB: " + BytesUtils.toHexString(certificate.getPublicKey().getEncoded(),
						Prefix.ZeroLowerX));
	...
	public void testVerifyCertificateFromRootPublicKey() {
		...
		logger.info("Certificate serial number: "
				+ BytesUtils.toHexString(certSerialNumBytes, Prefix.ZeroLowerX));
		byte[] rootPubKey = BytesUtils
				.toByteArray("0x036311FBABB1216467D7D823CBB2D3B9DBFB843A7D91AD16B1ED71596E094E4BBF");
		byte[] chaincode = BytesUtils
				.toByteArray("0xD39E4AACAB973117A4E64E636E067DA715337508F4DDDEF14C0BD2AD6E598760");
		logger.info("Certificate signature: "
				+ BytesUtils.toHexString(certificate.getSignature(), Prefix.ZeroLowerX));
		boolean isVerified = X509CertificateUtils.verifyCertificateSignature(certificate, rootPubKey,
				chaincode);
	...
	public void testCertSignAndVerify() {
		...
		ToClause clause = TransactionClient.buildTransferToClause(
				Address.fromHexString("0x7567d83b7b8d80addcb281a71d54fc7b3364ffed"), amount,
				ToData.ZERO, token);
		...
		String publicKeyStr = recoverPublicKey(BytesUtils.toByteArray(sign),
				BytesUtils.toHexString(content, null));
		logger.info("recoverPublicKey:{}", publicKeyStr);

		logger.info("PUB: {}",
				BytesUtils.toHexString(X509CertificateUtils.extractPublicKey(x509Certificate), null));

		// boolean verifyTxSignature = X509CertificateUtils
		// .verifyTxSignature(BytesUtils.toHexString(CryptoUtils.sha256(content), null),
		...
		logger.info("public key: {}",
				BytesUtils.toHexString(X509CertificateUtils.extractPublicKey(x509Certificate),
						null));
	...
	public String sign(String certPrivateKey, byte[] certSha256Bytes, Object... params) {
		...
		logger.info("Signing hash: {}",
				BytesUtils.toHexString(CryptoUtils.sha256(allBytes), Prefix.ZeroLowerX));
