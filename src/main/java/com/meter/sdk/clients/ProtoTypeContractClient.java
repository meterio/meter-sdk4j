...
import com.meter.sdk.core.model.blockchain.ContractCall;
import com.meter.sdk.core.model.blockchain.ContractCallResult;
...
/**
 * ProtoType Contract is a native contract for user to do multiple parties
 * payment(mpp).
 * <a href="https://github.com/meter/thor/wiki/Prototype(CN)">Prototype</a>
 * <br>
 * What is ProtoType for?</br>
 *
 * In common scenario, if a sender A want to make a transaction to target B,
 * the sender A need to pay the transaction fee(gas).
 *
 * However, for some realistic reason, the target A want to pay the transaction
 * fee. Then the mpp can do such a thing.
 * The sender A can be a user of target
 * B({@linkplain #addUsers(Address[], Address[], int, byte, int, ECKeyPair)}),
 * then the target B can set user plan (credit and recovery) for all senders.
 * After all the things is done, then the sender A do transaction to target B,
 * if the fee is less than the credit,
 * the ProtoType native contract is going to book fee(gas) from target B's
 * account.
 *
 * <br>
 * How to use ProtoType?</br>
 * First, you must be the master of the to address. By call
 * {@link #setMasterAddress}, then you can query the master by
 * {@link #getMasterAddress(Address, Revision)}
 *
 * Second, you as a Master, you can set add user to user plan. This step is to
 * set candidate, the one you want to give
...
public class ProtoTypeContractClient extends TransactionClient {
     ...
     * 
     * @param target   required {@link Address} target address, means transfer to
     *                 address.
     ...
     */
    public static TransferResult setMasterAddress(Address[] targets, Address[] newMasters, int gas, byte gasCoef,
        ...
        for (int index = 0; index < targets.length; index++) {
            clauses[index] = ProtoTypeContract.buildToClause(ProtoTypeContract.ContractAddress, abi, token,
                    targets[index].toHexString(Prefix.ZeroLowerX), newMasters[index].toHexString(Prefix.ZeroLowerX));
    ...
    /**
     * Add user to user plan, the caller must be the owner or master of target
     * addresses.
     * 
     ...
     * @throws ClientIOException network error.
     */
    public static TransferResult addUsers(Address[] targets, Address[] users, int gas, byte gasCoef, int expiration,
            ECKeyPair keyPair, int token) throws ClientIOException {
        if (targets == null) {
            throw ClientArgumentException.exception("targets is null");
        }
        if (users == null) {
    ...
    /**
     * Remove user from target addresses, the caller must be the owner or master of
     * target addresses.
     * 
     * @param targets    {@link Address} arrays.
     * @param users      {@link Address} arrays.
     ...
     * @throws ClientIOException network error.
     */
    public static TransferResult removeUsers(Address[] targets, Address[] users, int gas, byte gasCoef, int expiration,
            ECKeyPair keyPair, int token) throws ClientIOException {
        if (targets == null) {
            throw ClientArgumentException.exception("targets is null");
        }
    ...
    public static TransferResult setCreditPlans(Address[] targets,
            ...
            int gas, byte gasCoef, int expiration, ECKeyPair keyPair, int token) throws ClientIOException {
        ...
        if (targets.length != credits.length || targets.length != recoveryRates.length) {
            throw ClientArgumentException
                    .exception("users.length must equal to credits.length and equal to recoveries.length");
        }
    ...
    public static TransferResult sponsor(Address[] targets, int gas, byte gasCoef, int expiration, ECKeyPair keyPair,
            int token) throws ClientIOException {
        if (targets == null) {
            throw ClientArgumentException.exception("targets is null");
        }
    ...
    /**
     * Select sponsors for targets addresses, the caller must be the owner or master
     * of target addresses.
     * 
     ...
     */
    public static TransferResult selectSponsor(Address[] targets, Address[] sponsors, int gas, byte gasCoef,
            int expiration, ECKeyPair keyPair, int token) throws ClientIOException {
        if (targets == null) {
            throw ClientArgumentException.exception("targets is null");
        }
