...
public class ContractParamEncoder {
		...
		switch (abiType) {
			case "string":
				if (!(param instanceof String)) {
					throw new IllegalArgumentException("Parameter format is not match! index:"
							+ index + " expect: "
				...
				if (!param.getClass().isArray()) {
					...
					try {
						...
						throw new IllegalArgumentException(
								"Parameter format is not match! index:" + index
										+ " expect: "
										+ abiType + ",but get "
	...
	public static String encode(String abiType, Object param) {
		try {
			...
			throw new UnsupportedOperationException(
					"Type cannot be encoded: " + abiType + " paramClass:"
							+ param.getClass().getSimpleName(),
					e);
		}
	}
	...
	public static String encodeAddress(String address) {
		...
		if (paramBytes.length < MAX_BYTE_LENGTH) {
			...
			System.arraycopy(paramBytes, 0, fillingZero, MAX_BYTE_LENGTH - paramBytes.length,
					paramBytes.length);
