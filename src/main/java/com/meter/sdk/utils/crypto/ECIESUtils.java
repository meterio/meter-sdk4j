...
public class ECIESUtils {
	 ...
	 * @throws Exception
	 */
	public static byte[] decrypt(String receiverPrivateKey, String shareSecretKey, byte[] cryptedMessage)
			throws Exception {
		...
		if (!ByteUtils.toHexString(calcD).equals(ByteUtils.toHexString(dBytes))) {
			logger.error("different d:{} calc:{}", ByteUtils.toHexString(dBytes),
					ByteUtils.toHexString(calcD));
	...
	public static ECPoint createECPointFromPublicKey(String receiverPublicKey) {
		...
		if (receiverPublicKeyBytes == null || receiverPublicKeyBytes.length != 65
				|| receiverPublicKeyBytes[0] != 0x04) {
